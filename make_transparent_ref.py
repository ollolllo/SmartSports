...
# 输出图片路径
output_path = "D:\\Program Files\\nginx-1.26.3\\html\\imgs\\7_transparent_ref.png"
...
try:
    ...
    print(f"成功打开输入图片: {input_path}")
    print(f"输入图片模式: {img.mode}")
    print(f"输入图片尺寸: {img.width}x{img.height}")
    
    # 调整输入图片尺寸以匹配参考图片
    if img.size != ref_img.size:
        img = img.resize(ref_img.size, Image.Resampling.LANCZOS)
        print(f"调整输入图片尺寸为: {img.width}x{img.height}")
